...
def merge_vars(base_hash:, hash:, merge: 'replace')
  if base_hash.is_a?(Hash) && hash.is_a?(Hash)
    return base_hash.deep_merge!(hash) if merge.casecmp('merge') == 0
    return base_hash.merge!(hash) if merge.casecmp('replace') == 0
  end
  {}
end
...
if ENV['PLAYBOOK']
  ...
  playbook.each do |play|
    play.key?('vars') && (vars = merge_vars(base_hash: vars, hash: play['vars'], merge: hash_behaviour))
  end
  if File.exist?('vars/main.yml')
    role_vars = YAML.load_file('vars/main.yml')
    vars = merge_vars(base_hash: vars, hash: role_vars, merge: hash_behaviour)
  end
  set_property vars
end
